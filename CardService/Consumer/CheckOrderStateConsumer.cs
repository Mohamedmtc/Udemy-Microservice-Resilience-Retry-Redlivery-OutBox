    ...
    public class CheckOrderStateConsumer : IConsumer<ICheckOrderStateCommand>
    {
        ...
        public async Task Consume(ConsumeContext<ICheckOrderStateCommand> context)
        {
            try
            {
                ...
                _logger.LogInformation($"{DateTime.Now}Check Order State Consumer started - OrderId: {context.Message.OrderId} ");
                throw new Exception("Retry pattern and Redelivery");
